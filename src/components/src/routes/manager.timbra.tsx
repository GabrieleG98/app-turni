...
export const Route = createFileRoute("/manager/timbra")({
  component: ManagerTimbraPage,
});

function ManagerTimbraPage() {
  const {
    inTurno, haGiaSessioni, busy, clockIn, clockOut,
    windowState, minutiRitardo, canClock, turnoOggi,
    conferma, closeConferma,
  } = useTimbratura();
  ...
  const tooltip =
    windowState === "no-shift" ? "Nessun turno schedulato per oggi"
    : windowState === "too-early" && turnoOggi
    ? `Disponibile dalle ${turnoOggi.ora_inizio.slice(0, 5)} (5 min prima)`
    : windowState === "missed" ? "Turno passato — chiedi una correzione al manager"
    : "";

  const label = inTurno ? "Termina sessione" : haGiaSessioni ? "Nuova sessione" : "Inizia sessione";
  const Icon = inTurno ? Square
    : haGiaSessioni ? RotateCw
    : windowState === "too-early" || windowState === "no-shift" ? Clock
    : Play;
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-8 space-y-6">
      ...
      {turnoOggi && (
        <Card className="p-4">
          <div className="text-xs uppercase tracking-wider text-muted-foreground font-semibold mb-1">Turno di oggi</div>
          <div className="font-bold text-lg capitalize">{turnoOggi.tipo_turno}</div>
          <div>{turnoOggi.ora_inizio.slice(0, 5)} – {turnoOggi.ora_fine.slice(0, 5)}</div>
        </Card>
      )}
      ...
      <Card className="p-6 flex flex-col items-center gap-4">
        <div className={cn(
          ...
          : inTurno ? "bg-destructive text-destructive-foreground"
          ...
          {isLate && <span className="text-xs font-bold">{fmtRitardo(minutiRitardo)}</span>}
        </div>
