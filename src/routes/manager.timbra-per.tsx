...
import { ArrowLeft, Play, Square, Loader2, Camera } from "lucide-react";
...
function SelfieDialog({ url, nome, open, onClose }: {
  url: string | null;
  nome: string;
  open: boolean;
  onClose: () => void;
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>Selfie — {nome}</DialogTitle>
        </DialogHeader>
        {url ? (
          <img
            src={url}
...
function TimbraPerPage() {
  ...
  const [selfieDialog, setSelfieDialog] = useState<{ url: string | null; nome: string } | null>(null);
  ...
  const openSelfie = async (sess: any, nome: string) => {
    const path = sess.foto_in_url || sess.foto_out_url || null;
    if (!path) {
      setSelfieDialog({ url: null, nome });
      return;
    }
    const url = await getSelfieSignedUrl(path);
    setSelfieDialog({ url: url ?? null, nome });
  };
  ...
  return (
    <div className="space-y-4 max-w-5xl">
      {selfieDialog && (
        <SelfieDialog
          url={selfieDialog.url}
          nome={selfieDialog.nome}
          open={true}
          onClose={() => setSelfieDialog(null)}
        />
      )}
      ...
      <div className="grid gap-3 sm:hidden">
        {profili.map((p) => {
          ...
          const ultima = sess[sess.length - 1];
          const turno = turnoOf(p.id);
          const hasFoto = ultima && (ultima.foto_in_url || ultima.foto_out_url);
          return (
              ...
              <div className="flex items-start justify-between">
                ...
                {hasFoto && (
                  <Button
                    ...
                    onClick={() => openSelfie(ultima, `${p.nome} ${p.cognome}`)}
                  >
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-xs uppercase">
            <tr>
              ...
              <th className="text-center p-3">Selfie</th>
